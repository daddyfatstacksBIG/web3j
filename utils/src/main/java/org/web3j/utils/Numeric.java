...
public final class Numeric {
  ...
  private static String toHexStringZeroPadded(BigInteger value, int size,
                                              boolean withPrefix) {
    String result = toHexStringNoPrefix(value);

    int length = result.length();
    if (length > size) {
      throw new UnsupportedOperationException("Value " + result +
                                              "is larger then length " + size);
  ...
  public static byte[] hexStringToByteArray(String input) {
    ...
    for (int i = startIdx; i < len; i += 2) {
      data[(i + 1) / 2] =
          (byte)((Character.digit(cleanInput.charAt(i), 16) << 4) +
  ...
  public static byte asByte(int m, int n) { return (byte)((m << 4) | n); }
