...
public class Base64String {
  ...
  public static class Serializer extends JsonSerializer {
    @Override
    public void serialize(final Object value, JsonGenerator gen,
  ...
  public static List<Base64String> wrapList(final List<String> base64Strings) {
    return base64Strings.stream()
        .map(Base64String::wrap)
        .collect(Collectors.toList());
  }

  public static List<Base64String> wrapList(final String... base64Strings) {
    return Arrays.stream(base64Strings)
        .map(Base64String::wrap)
        .collect(Collectors.toList());
  }

  public static List<String>
  unwrapList(final List<Base64String> base64Strings) {
    return base64Strings.stream()
        .map(Base64String::toString)
        .collect(Collectors.toList());
  }

  public static RlpList
  unwrapListToRlp(final List<Base64String> base64Strings) {
    return new RlpList(base64Strings.stream()
                           .map(Base64String::asRlp)
                           .collect(Collectors.toList()));
  }
  ...
  private Base64String(final String base64String) {
    if (!isValid(base64String) || base64String.length() != 44) {
      throw new IllegalArgumentException(base64String +
                                         " is not a 32 byte base 64 value");
    }

    System.arraycopy(DECODER.decode(base64String), 0, enclaveB64Value, 0, 32);
  }

  public String toString() { return ENCODER.encodeToString(enclaveB64Value); }

  public byte[] raw() { return enclaveB64Value; }

  public RlpString asRlp() { return RlpString.create(raw()); }
