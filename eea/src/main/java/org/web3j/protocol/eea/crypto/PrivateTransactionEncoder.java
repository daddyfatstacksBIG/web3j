...
public class PrivateTransactionEncoder {
  ...
  public static byte[] signMessage(final RawPrivateTransaction rawTransaction,
  ...
  public static List<RlpType>
  asRlpValues(final RawPrivateTransaction privateTransaction,
              final Sign.SignatureData signatureData) {

    final List<RlpType> result = new ArrayList<>(TransactionEncoder.asRlpValues(
    ...
    privateTransaction.getPrivateFor().ifPresent(
        privateFor -> result.add(Base64String.unwrapListToRlp(privateFor)));
    ...
    result.add(
        RlpString.create(privateTransaction.getRestriction().getRestriction()));
