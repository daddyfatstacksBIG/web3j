 ...
 *
 * <p>For the specification, refer to p16 of the <a
...
public class RlpDecoder {
  ...
  /**
   * [0xb7] If a string is more than 55 bytes long, the RLP encoding consists of
   * a single byte with value 0xb7 plus the length of the length of the string
   * in binary form, followed by the length of the string, followed by the
   * string. For example, a length-1024 string would be encoded as \xb9\x04\x00
   ...
   * RLP encodings of the items. The range of the first byte is thus [0xc0,
   * 0xf7].
   */
   ...
   * followed by the concatenation of the RLP encodings of the items. The range
  ...
  private static void traverse(byte[] data, int startPos, int endPos,
                               RlpList rlpList) {
    ...
    try {
      ...
      while (startPos < endPos) {
        ...
        if (prefix < OFFSET_SHORT_STRING) {
          ...
          byte[] rlpData = new byte[strLen];
          System.arraycopy(data, startPos + lenOfStrLen + 1, rlpData, 0,
                           strLen);
          ...
          // total payload is equal to the first byte minus 0xc0 follows the
          // first byte;
