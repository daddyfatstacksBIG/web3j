...
public class RlpEncoderTest {
  ...
  public void testEncode() {
    assertEquals(RlpEncoder.encode(RlpString.create("dog")),
                 (new byte[] {(byte)0x83, 'd', 'o', 'g'}));

    assertEquals(RlpEncoder.encode(new RlpList(RlpString.create("cat"),
                                               RlpString.create("dog"))),
                 (new byte[] {(byte)0xc8, (byte)0x83, 'c', 'a', 't', (byte)0x83,
                              'd', 'o', 'g'}));

    assertEquals(RlpEncoder.encode(RlpString.create("")),
                 (new byte[] {(byte)0x80}));

    assertEquals(RlpEncoder.encode(RlpString.create(new byte[] {})),
                 (new byte[] {(byte)0x80}));

    assertEquals(RlpEncoder.encode(new RlpList()), (new byte[] {(byte)0xc0}));

    assertEquals(RlpEncoder.encode(RlpString.create(BigInteger.valueOf(0x0f))),
                 (new byte[] {(byte)0x0f}));
    ...
    assertEquals(RlpEncoder.encode(RlpString.create(BigInteger.ZERO)),
                 (new byte[] {(byte)0x80}));

    // https://github.com/paritytech/parity-common/blob/master/rlp/tests/tests.rs#L237
    assertEquals(RlpEncoder.encode(RlpString.create(new byte[] {0})),
                 (new byte[] {(byte)0x00}));

    assertEquals(RlpEncoder.encode(new RlpList(RlpString.create("zw"),
                                               new RlpList(RlpString.create(4)),
                                               RlpString.create(1))),
                 (new byte[] {(byte)0xc6, (byte)0x82, (byte)0x7a, (byte)0x77,
    ...
    assertEquals(RlpEncoder.encode(RlpString.create(encodeMe)),
                 (expectedEncoding));
  }
}
