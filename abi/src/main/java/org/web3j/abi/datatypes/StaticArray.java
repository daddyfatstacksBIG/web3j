...
public abstract class StaticArray<T extends Type> extends Array<T> {
  ...
  /**
   * Warning: increasing this constant will cause more generated StaticArrayN
   * types, see: org.web3j.codegen.AbiTypesGenerator#generateStaticArrayTypes
   */
  ...
  private void checkValid(int expectedSize) {
    if (value.size() > MAX_SIZE_OF_STATIC_ARRAY) {
      ...
      throw new UnsupportedOperationException(
          "Expected array of type [" + getClass().getSimpleName() +
          "] to have [" + expectedSize + "] elements.");
    }
  }
}
