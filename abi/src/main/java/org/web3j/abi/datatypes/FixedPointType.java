...
public abstract class FixedPointType extends NumericType {
  ...
  public FixedPointType(String typePrefix, int mBitSize, int nBitSize,
                        BigInteger value) {
  ...
  private boolean isValidBitSize(int mBitSize, int nBitSize) {
    return mBitSize % 8 == 0 && nBitSize % 8 == 0 && bitSize > 0 &&
        bitSize <= MAX_BIT_LENGTH;
  }

  private static boolean isValidBitCount(int mBitSize, int nBitSize,
                                         BigInteger value) {
  ...
  static BigInteger convert(int mBitSize, int nBitSize, BigInteger m,
                            BigInteger n) {
