...
/**
 * Type wrapper to get around limitations of Java's type erasure. This is so
 * that we can pass around Typed {@link org.web3j.abi.datatypes.Array} types.
 *
 * <p>See <a
 * href="http://gafter.blogspot.com.au/2006/12/super-type-tokens.html">this blog
 * post</a> for further details.
 *
 * <p>It may make sense to switch to using Java's reflection <a
 * href="https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Type.html">Type</a>
 * to avoid working around this fundamental generics limitation.
 */
public abstract class TypeReference<T extends org.web3j.abi.datatypes.Type>
  ...
  protected TypeReference() { this(false); }
   ...
   * to DynamicArray[StaticArray3[Uint256]] would return a TypeReference to
   * StaticArray3[Uint256]
   *
   * @return the type wrapped by this Array TypeReference, or null if not Array
   */
  TypeReference getSubTypeReference() { return null; }
  ...
  /**
   * Workaround to ensure type does not come back as T due to erasure, this
   * enables you to create a TypeReference via {@link Class Class&lt;T&gt;}.
   *
   * @return the parameterized Class type if applicable, otherwise a regular
   *     class
  ...
  public static <T extends org.web3j.abi.datatypes.Type> TypeReference<T>
  create(Class<T> cls) {
    return create(cls, false);
  }

  public static <T extends org.web3j.abi.datatypes.Type> TypeReference<T>
  create(Class<T> cls, boolean indexed) {
    return new TypeReference<T>(indexed) {
      public java.lang.reflect.Type getType() { return cls; }
    };
  }

  /**
   * This is a helper method that only works for atomic types (uint, bytes,
   * etc). Array types must be wrapped by a {@link
  ...
  getAtomicTypeClass(String solidityType, boolean primitives)
  ...
  public abstract static class StaticArrayTypeReference<
      T extends org.web3j.abi.datatypes.Type> extends TypeReference<T> {

    private final int size;

    protected StaticArrayTypeReference(int size) { this.size = size; }
