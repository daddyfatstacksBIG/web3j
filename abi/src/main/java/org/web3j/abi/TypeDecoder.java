 ...
 * Decoding is not documented, but is the reverse of the encoding details
 * located <a
...
public class TypeDecoder {
  ...
  public static <T extends Array> T decode(String input, int offset,
    ...
    Class cls =
        ((ParameterizedType)typeReference.getType()).getRawType().getClass();
    if (StaticArray.class.isAssignableFrom(cls)) {
      ...
      throw new UnsupportedOperationException(
          "Unsupported TypeReference: " + cls.getName() +
  ...
  static <T extends Type> T decode(String input, int offset, Class<T> type) {
    if (NumericType.class.isAssignableFrom(type)) {
      return (T)decodeNumeric(input.substring(offset),
      ...
      throw new UnsupportedOperationException("Type cannot be encoded: " +
                                              type.getClass());
    }
  }
  ...
  static <T extends NumericType> int getTypeLength(Class<T> type) {
    if (IntType.class.isAssignableFrom(type)) {
      String regex = "(" + Uint.class.getSimpleName() + "|" +
  ...
  static Type instantiateArrayType(TypeReference ref, Object value)
    ...
    // create a list of arguments coerced to the correct type of
    // sub-TypeReference
    ...
    return (Type)listcons.newInstance(subTypeReference.getClassType(),
                                      transformedList);
  }
  ...
  static Type instantiateAtomicType(Class<?> referenceClass, Object value)
    ...
    if (constructorArg == null) {
      throw new InstantiationException(
          "Could not create type " + referenceClass + " from arg " +
  ...
  static <T extends Type> int getSingleElementLength(String input, int offset,
                                                     Class<T> type) {
  ...
  static int decodeUintAsInt(String rawInput, int offset) {
    String input =
        rawInput.substring(offset, offset + MAX_BYTE_LENGTH_FOR_HEX_STRING);
  ...
  static <T extends Bytes> T decodeBytes(String input, int offset,
                                         Class<T> type) {
    try {
  ...
  static DynamicBytes decodeDynamicBytes(String input, int offset) {
    ...
    String data =
        input.substring(valueOffset, valueOffset + hexStringEncodedLength);
  ...
  @SuppressWarnings("unchecked")
  static <T extends Type> T decodeStaticArray(String input, int offset,
                                              TypeReference<T> typeReference,
                                              int length) {
  ...
  static <T extends Type> T decodeDynamicArray(String input, int offset,
    ...
    BiFunction<List<T>, String, T> function = (elements, typeName)
        -> (T) new DynamicArray(AbiTypes.getType(typeName), elements);

    int valueOffset = offset + MAX_BYTE_LENGTH_FOR_HEX_STRING;

    return decodeArrayElements(input, valueOffset, typeReference, length,
                               function);
  }
  ...
  private static <T extends Type>
      T instantiateStaticArray(TypeReference<T> typeReference, List<T> elements,
                               int length) {
    try {
  ...
  private static <T extends Type>
      T decodeArrayElements(String input, int offset,
                            TypeReference<T> typeReference, int length,
    ...
    try {
      ...
      if (Array.class.isAssignableFrom(cls)) {
        ...
        for (int i = 0, currOffset = offset; i < length;
