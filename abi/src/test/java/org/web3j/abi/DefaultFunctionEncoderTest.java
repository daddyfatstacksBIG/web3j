...
public class DefaultFunctionEncoderTest {
  ...
  public void testBuildMethodId() {
    assertEquals("0xcdcd77c0",
                 DefaultFunctionEncoder.buildMethodId("baz(uint32,bool)"));
  }

  @Test
  public void testBuildMessageSignature() {
    assertEquals("baz(uint32,bool)",
                 DefaultFunctionEncoder.buildMethodSignature(
                     "baz", Arrays.asList(new Uint32(BigInteger.valueOf(69)),
                                          new Bool(true))));
  }

  @Test
  public void testBuildEmptyMethodSignature() {
    assertEquals("empty()", DefaultFunctionEncoder.buildMethodSignature(
  ...
  public void testFunctionMDynamicArrayEncode1() {
    Function function = new Function(
        ...
        Arrays.asList(new DynamicBytes("dave".getBytes()), new Bool(true),
                      new DynamicArray<>(new Uint(BigInteger.ONE),
  ...
  public void testFunctionMDynamicArrayEncode2() {
    Function function = new Function(
        "f",
        Arrays.asList(new Uint(BigInteger.valueOf(0x123)),
                      new DynamicArray<>(new Uint32(BigInteger.valueOf(0x456)),
  ...
  public void testMakeFunction()
    ...
    Function expectedFunction =
        new Function("function", Arrays.asList(new Bool(true)),
                     Arrays.asList(new TypeReference<Uint256>() {}));

    Function actualFunction = FunctionEncoder.makeFunction(
        "function", Arrays.asList("bool"), Arrays.asList(true),
        Arrays.asList("uint256"));

    assertEquals(actualFunction.getName(), expectedFunction.getName());

    Iterator<Type> expectedInput =
        expectedFunction.getInputParameters().iterator();
    ...
    for (TypeReference<Type> actualOutput :
         actualFunction.getOutputParameters()) {
