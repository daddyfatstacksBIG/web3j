...
public class BesuOnChainPrivacyIntegrationTest {
  ...
  public void testCreateAndFindOnChainPrivacyGroup() throws Exception {
    ...
    final String txHash = nodeAlice
                              .privOnChainCreatePrivacyGroup(
                                  privacyGroupId, ALICE, ENCLAVE_KEY_ALICE,
    ...
    List<PrivacyGroup> groups = nodeAlice
                                    .privOnChainFindPrivacyGroup(Arrays.asList(
                                        ENCLAVE_KEY_ALICE, ENCLAVE_KEY_BOB))
                                    .send()
                                    .getGroups();

    assertTrue(groups.stream().anyMatch(
        x -> x.getPrivacyGroupId().equals(privacyGroupId)));
  }
  ...
  public void testCreateAddRemoveOnChainPrivacyGroup() throws Exception {
    ...
    TransactionReceipt createReceipt =
        processor.waitForTransactionReceipt(createTxHash);
    ...
    TransactionReceipt addReceipt =
        processor.waitForTransactionReceipt(addTxHash);
    ...
    TransactionReceipt removeReceipt =
        processor.waitForTransactionReceipt(removeTxHash);
    ...
    List<PrivacyGroup> removedGroups =
        nodeAlice
            ...
            .send()
            .getGroups();
    assertTrue(removedGroups.stream().anyMatch(
        g
        -> g.getPrivacyGroupId().equals(privacyGroupId) &&
               g.getMembers().size() == 2));
  }
  ...
  public void testCannotAddToAlreadyLockedGroup() throws Exception {
    ...
    final String lockTxHash =
        nodeAlice
            .privOnChainSetGroupLockState(privacyGroupId, ALICE,
                                          ENCLAVE_KEY_ALICE, true)
            .send()
            .getTransactionHash();

    TransactionReceipt lockReceipt =
        processor.waitForTransactionReceipt(lockTxHash);
    ...
    assertThrows(TransactionException.class,
                 ()
                     -> nodeBob
                            .privOnChainAddToPrivacyGroup(
                                privacyGroupId, BOB, ENCLAVE_KEY_BOB,
                                Collections.singletonList(ENCLAVE_KEY_CHARLIE))
                            .send());
  }

  @Test
  public void testCanExecuteSmartContractsInOnChainPrivacyGroup()
      throws Exception {
    Base64String aliceBobGroup = Base64String.wrap(generateRandomBytes(32));
    final String createTxHash =
        nodeAlice
            .privOnChainCreatePrivacyGroup(
                aliceBobGroup, ALICE, ENCLAVE_KEY_ALICE,
                Collections.singletonList(ENCLAVE_KEY_BOB))
            .send()
            .getTransactionHash();
