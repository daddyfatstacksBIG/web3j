...
public abstract class PrivateTransactionManager extends TransactionManager {
  private static final Logger log =
      LoggerFactory.getLogger(PrivateTransactionManager.class);
  ...
  protected PrivateTransactionManager(
      final Besu besu, final BesuPrivacyGasProvider gasProvider,
      final Credentials credentials, final long chainId,
  ...
  protected PrivateTransactionManager(final Besu besu,
    ...
    this(besu, gasProvider, credentials, chainId, privateFrom,
         new PollingPrivateTransactionReceiptProcessor(besu, attempts,
                                                       sleepDuration));
  }
  ...
  protected PrivateTransactionManager(final Besu besu,
    ...
    this(besu, gasProvider, credentials, chainId, privateFrom,
         new PollingPrivateTransactionReceiptProcessor(
             besu, DEFAULT_POLLING_FREQUENCY,
             DEFAULT_POLLING_ATTEMPTS_PER_TX_HASH));
  }

  @Override
  protected TransactionReceipt
  executeTransaction(BigInteger gasPrice, BigInteger gasLimit, String to,
  ...
  public Base64String getPrivateFrom() { return privateFrom; }
  ...
  sendTransaction(final BigInteger gasPrice, final BigInteger gasLimit,
                  final String to, final String data, final BigInteger value,
                  boolean constructor) throws IOException {

    final BigInteger nonce =
        besu.privGetTransactionCount(credentials.getAddress(),
                                     getPrivacyGroupId())
            .send()
            .getTransactionCount();
    ...
    if (privacyGroupIdOrPrivateFor instanceof Base64String) {
      transaction = RawPrivateTransaction.createTransaction(
          nonce, gasPrice, gasLimit, to, data, privateFrom,
  ...
  public EthSendTransaction sendTransactionEIP1559(
    ...
    final RawPrivateTransaction transaction;
    if (privacyGroupIdOrPrivateFor instanceof Base64String) {
      transaction = RawPrivateTransaction.createTransactionEIP1559(
          nonce, gasPremium, feeCap, gasLimit, to, data, privateFrom,
          (Base64String)privacyGroupIdOrPrivateFor, RESTRICTED);
    } else {
      transaction = RawPrivateTransaction.createTransactionEIP1559(
          nonce, gasPremium, feeCap, gasLimit, to, data, privateFrom,
  ...
  processResponse(final EthSendTransaction transactionResponse)
      throws IOException, TransactionException {
    ...
    return transactionReceiptProcessor.waitForTransactionReceipt(
        transactionHash);
  }
  ...
  public EthGetCode getCode(final String contractAddress,
      ...
      throws IOException {
    return this.besu
        .privGetCode(this.getPrivacyGroupId().toString(), contractAddress,
