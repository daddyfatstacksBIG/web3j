...
public interface Besu extends Eea, BesuRx {
   ...
   * @param scheduledExecutorService executor service to use for scheduled
   *     tasks. <strong>You are responsible for terminating this thread
   *     pool</strong>
   * @return new Besu instance
   */
  ...
  Request<?, BooleanResponse> cliquePropose(String address,
                                            Boolean signerAddition);

  Request<?, BesuEthAccountsMapResponse> cliqueProposals();

  Request<?, BesuFullDebugTraceResponse>
  ...
  Request<?, PrivGetPrivateTransaction>
  ...
  Request<?, PrivCreatePrivacyGroup>
  ...
  Request<?, EthSendTransaction> privOnChainCreatePrivacyGroup(
      Base64String privacyGroupId, Credentials credentials,
      Base64String enclaveKey, List<Base64String> participants)
      throws IOException;
  ...
  Request<?, PrivGetTransactionReceipt>
  privGetTransactionReceipt(final String transactionHash);

  Request<?, EthGetCode>
  privGetCode(String privacyGroupId, String address,
              DefaultBlockParameter defaultBlockParameter);

  Request<?, org.web3j.protocol.core.methods.response.EthCall>
  privCall(String privacyGroupId,
           org.web3j.protocol.core.methods.request.Transaction transaction,
           DefaultBlockParameter defaultBlockParameter);

  Request<?, EthLog>
  privGetLogs(String privacyGroupId,
  ...
  Request<?, EthUninstallFilter> privUninstallFilter(String privacyGroupId,
                                                     String filterId);

  Request<?, EthLog> privGetFilterChanges(String privacyGroupId,
                                          String filterId);
