...
public class PrivateLogFilter extends LogFilter {
  ...
  public PrivateLogFilter(
      Besu web3j, Callback<Log> callback, String privacyGroupId,
  ...
  protected EthUninstallFilter uninstallFilter(BigInteger filterId)
      throws IOException {
    return ((Besu)web3j)
        .privUninstallFilter(privacyGroupId,
                             Numeric.toHexStringWithPrefix(filterId))
        .send();
  }
  ...
  protected Optional<Request<?, EthLog>> getFilterLogs(BigInteger filterId) {
    return Optional.of(
        ((Besu)web3j)
            .privGetFilterLogs(privacyGroupId,
                               Numeric.toHexStringWithPrefix(filterId)));
  }
}
