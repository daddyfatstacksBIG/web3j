...
public abstract class PrivateTransaction {
  ...
  public static class ResponseDeserialiser
      extends StdDeserializer<PrivateTransaction> {

    protected ResponseDeserialiser() { super(PrivateTransaction.class); }

    @Override
    public PrivateTransaction deserialize(JsonParser p,
      ...
      } else if ((node.get("privateFor") != null &&
                  node.get("privateFor").isValueNode()) ||
                 (node.get("privacyGroupId") != null &&
                  node.get("privacyGroupId").isValueNode())) {
        return p.getCodec().treeToValue(
            node, PrivateTransactionWithPrivacyGroup.class);
      }
  ...
  public PrivateTransaction(final String hash, final String nonce,
                            final String from, final String to,
                            final String value, final String gas,
                            final String gasPrice, final String input,
                            final String r, final String s, final String v,
  ...
  public boolean equals(Object o) {
    ...
    return getV() == that.getV() && getHash().equals(that.getHash()) &&
        getNonce().equals(that.getNonce()) &&
        getFrom().equals(that.getFrom()) && getTo().equals(that.getTo()) &&
        getValue().equals(that.getValue()) &&
        getGasPrice().equals(that.getGasPrice()) &&
        getGas().equals(that.getGas()) && getInput().equals(that.getInput()) &&
        getR().equals(that.getR()) && getS().equals(that.getS()) &&
  ...
  @Override
  public int hashCode() {
    return Objects.hash(getHash(), getNonce(), getFrom(), getTo(), getValue(),
                        getGasPrice(), getGas(), getInput(), getR(), getS(),
