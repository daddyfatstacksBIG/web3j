...
public class TransactionDecoder {
  ...
  public static RawTransaction decode(final String hexTransaction) {
    ...
    final BigInteger nonce =
        ((RlpString)values.getValues().get(0)).asPositiveBigInteger();
    final BigInteger gasPrice =
        ((RlpString)values.getValues().get(1)).asPositiveBigInteger();
    final BigInteger gasLimit =
        ((RlpString)values.getValues().get(2)).asPositiveBigInteger();
    final String to = ((RlpString)values.getValues().get(3)).asString();
    final BigInteger value =
        ((RlpString)values.getValues().get(4)).asPositiveBigInteger();
    ...
    if (values.getValues().size() == 6 ||
      ...
      return RawTransaction.createTransaction(nonce, gasPrice, gasLimit, to,
                                              value, data);
    } else {
      ...
      return new SignedRawTransaction(nonce, gasPrice, gasLimit, to, value,
                                      data, signatureData);
    }
  }
}
