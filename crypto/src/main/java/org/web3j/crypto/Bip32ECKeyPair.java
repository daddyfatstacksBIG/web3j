...
public class Bip32ECKeyPair extends ECKeyPair {
  ...
  public Bip32ECKeyPair(BigInteger privateKey, BigInteger publicKey,
                        int childNumber, byte[] chainCode,
  ...
  public static Bip32ECKeyPair deriveKeyPair(Bip32ECKeyPair master,
                                             int[] path) {
  ...
  private Bip32ECKeyPair deriveChildKey(int childNumber) {
    if (!hasPrivateKey()) {
      ...
      return new Bip32ECKeyPair(null, Sign.publicFromPoint(ki.getEncoded(true)),
      ...
      return new Bip32ECKeyPair(privateKey,
  ...
  private int getFingerprint() {
    ...
    return id[3] & 0xFF | (id[2] & 0xFF) << 8 | (id[1] & 0xFF) << 16 |
        (id[0] & 0xFF) << 24;
  }
  ...
  private static boolean isHardened(int a) { return (a & HARDENED_BIT) != 0; }
}
