...
public class ECKeyPair {
  ...
  public ECDSASignature sign(byte[] transactionHash) {
    ...
    ECPrivateKeyParameters privKey =
        new ECPrivateKeyParameters(privateKey, Sign.CURVE);
  ...
  public static ECKeyPair create(KeyPair keyPair) {
    ...
    // details Additionally, as the first bit is a constant prefix (0x04) we
    // ignore this value
    byte[] publicKeyBytes = publicKey.getQ().getEncoded(false);
    BigInteger publicKeyValue = new BigInteger(
        1, Arrays.copyOfRange(publicKeyBytes, 1, publicKeyBytes.length));
  ...
  public boolean equals(Object o) {
    ...
    if (privateKey != null ? !privateKey.equals(ecKeyPair.privateKey)
                           : ecKeyPair.privateKey != null) {
      return false;
    }

    return publicKey != null ? publicKey.equals(ecKeyPair.publicKey)
