...
public class Keys {
  ...
  public static String getAddress(BigInteger publicKey) {
    return getAddress(Numeric.toHexStringWithPrefixZeroPadded(
  ...
  public static String getAddress(String publicKey) {
    ...
    return hash.substring(hash.length() -
                          ADDRESS_LENGTH_IN_HEX); // right most 160 bits
  }

  public static byte[] getAddress(byte[] publicKey) {
    byte[] hash = Hash.sha3(publicKey);
    return Arrays.copyOfRange(hash, hash.length - 20,
                              hash.length); // right most 160 bits
  }
  ...
  public static String toChecksumAddress(String address) {
    ...
    String addressHash =
        Numeric.cleanHexPrefix(Hash.sha3String(lowercaseAddress));
  ...
  public static byte[] serialize(ECKeyPair ecKeyPair) {
    ...
    byte[] publicKey =
        Numeric.toBytesPadded(ecKeyPair.getPublicKey(), PUBLIC_KEY_SIZE);

    byte[] result =
        Arrays.copyOf(privateKey, PRIVATE_KEY_SIZE + PUBLIC_KEY_SIZE);
  ...
  public static ECKeyPair deserialize(byte[] input) {
    ...
    BigInteger publicKey =
        Numeric.toBigInt(input, PRIVATE_KEY_SIZE, PUBLIC_KEY_SIZE);
