...
public class Sign {
  ...
  public static SignatureData signPrefixedMessage(byte[] message,
                                                  ECKeyPair keyPair) {
  ...
  public static SignatureData signMessage(byte[] message, ECKeyPair keyPair,
    ...
    // Now we have to work backwards to figure out the recId needed to recover
    // the signature.
    int recId = -1;
    for (int i = 0; i < 4; i++) {
   ...
   * possible keys is the correct one. Because the key recovery operation yields
   ...
   *
   * <p>If this method returns null it means recovery was not possible and recId
   * should be iterated.
   *
   * <p>Given the above two points, a correct usage of this method is inside a
   ...
   *     wasn't possible.
   */
  public static BigInteger recoverFromSignature(int recId, ECDSASignature sig,
                                                byte[] message) {
    ...
    //   the conversion
    //        routine specified in Section 2.3.7, where mlen = ⌈(log2 p)/8⌉ or
    //        mlen = ⌈m/8⌉.
    ...
    //        conversion routine outputs "invalid", then do another iteration of
    //        Step 1.
    //
    ...
    //               Q = mi(r) * (sR - eG)
    //
    // Where mi(x) is the modular multiplicative inverse. We transform this into
    // the following:
    //               Q = (mi(r) * s ** R) + (mi(r) * -e ** G)
    // Where -e is the modular additive inverse of e, that is z such that z + e
    // = 0 (mod n). In the above equation ** is point multiplication and + is
    // point addition (the EC group operator).
    //
    // We can find the additive inverse by subtracting e from zero then taking
    // the mod. For example the additive inverse of 3 modulo 11 is 8 because 3 +
    // 8 mod 11 = 0, and -3 mod 11 = 8.
  ...
  /**
   * Given an arbitrary piece of text and an Ethereum message signature encoded
   * in bytes, returns the public key that was used to sign it. This can then be
   ...
   */
  public static BigInteger signedMessageToKey(byte[] message,
  ...
  signedPrefixedMessageToKey(byte[] message, SignatureData signatureData)
      throws SignatureException {
    return signedMessageHashToKey(getEthereumMessageHash(message),
                                  signatureData);
  }

  /**
   * Given an arbitrary message hash and an Ethereum message signature encoded
   * in bytes, returns the public key that was used to sign it. This can then be
   * compared to the expected public key to determine if the signature was
   * correct.
   *
   ...
   *     there was a signature format error.
   */
  public static BigInteger signedMessageHashToKey(byte[] messageHash,
                                                  SignatureData signatureData)
      throws SignatureException {
  ...
  public static BigInteger publicKeyFromPrivate(BigInteger privKey) {
    ...
    return new BigInteger(
        1, Arrays.copyOfRange(encoded, 1, encoded.length)); // remove prefix
  }
  ...
  public static ECPoint publicPointFromPrivate(BigInteger privKey) {
    /*
     * TODO: FixedPointCombMultiplier currently doesn't support scalars longer
     * than the group order, but that could change in future versions.
     */
  ...
  public static BigInteger publicFromPoint(byte[] bits) {
    return new BigInteger(
        1, Arrays.copyOfRange(bits, 1, bits.length)); // remove prefix
  }
