...
/**
 * Provides utility methods to generate random mnemonics and also generate seeds
 * from mnemonics.
 *
 * @see <a
 *     href="https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki">Mnemonic
 *     code for generating deterministic keys</a>
 */
public class MnemonicUtils {
   ...
   * entropy security is improved but the sentence length increases. We refer to
   * the initial entropy length as ENT. The allowed size of ENT is 128-256 bits.
   *
   * <h3>Mnemonic generation algorithm</h3>
   *
   * Given a randomly generated initial entropy of size ENT, first a checksum is
   * generated by taking the first {@code ENT / 32} bits of its SHA256 hash.
   * This checksum is appended to the end of the initial entropy. Next, these
   * concatenated bits are split into groups of 11 bits, each encoding a number
   * from 0-2047, serving as an index into a wordlist. Finally, we convert these
   * numbers into words and use the joined words as a mnemonic sentence.
   *
   ...
   *
   * @param mnemonic The input mnemonic which should be 128-160 bits in length
   ...
   * "mnemonic" + passphrase (again in UTF-8 NFKD) used as the salt. The
   ...
   * @param mnemonic The input mnemonic which should be 128-160 bits in length
   *     containing only valid words
   * @param passphrase The passphrase which will be used as part of salt for
   *     PBKDF2 function
  ...
  public static byte[] generateSeed(String mnemonic, String passphrase) {
    ...
    return ((KeyParameter)gen.generateDerivedParameters(SEED_KEY_SIZE))
        .getKey();
  }
  ...
  private static void validateEntropy(byte[] entropy) {
    ...
    if (ent < 128 || ent > 256 || ent % 32 != 0) {
      throw new IllegalArgumentException(
          "The allowed size of ENT is 128-256 bits of "
          + "multiples of 32");
    }
  }
  ...
  private static boolean isBitSet(int n, int k) { return ((n >> k) & 1) == 1; }
