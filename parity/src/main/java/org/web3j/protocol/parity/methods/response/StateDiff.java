...
public class StateDiff {
  ...
  public static class ChangedState implements State {
    ...
    public boolean equals(Object o) {
      ...
      return getTo() != null ? getTo().equals(that.getTo())
                             : that.getTo() == null;
    }
  ...
  public static class AddedState implements State {
    ...
    public AddedState(String value) { this.value = value; }
  ...
  public StateDiff(State balance, State code, State nonce,
                   Map<String, State> storage) {
  ...
  public State getBalance() { return balance; }

  public void setBalance(JsonNode balance) {
    this.balance = deserializeState(balance);
  }

  public State getCode() { return code; }

  public void setCode(JsonNode code) { this.code = deserializeState(code); }

  public State getNonce() { return nonce; }

  public void setNonce(JsonNode nonce) { this.nonce = deserializeState(nonce); }

  public Map<String, State> getStorage() { return storage; }

  public void setStorage(Map<String, JsonNode> storage) {
    this.storage = storage.entrySet().stream().collect(Collectors.toMap(
        Map.Entry::getKey, entry -> deserializeState(entry.getValue())));
  }
  ...
  private State deserializeState(JsonNode node) {
    ...
    if (node.isTextual() && node.asText().equals("=")) {
      ...
      if (subNode.isObject() && subNode.has("from") && subNode.has("to")) {
        state = new ChangedState(subNode.get("from").asText(),
  ...
  public String toString() {
    return "StateDiff{"
        + "balance=" + getBalance() + ", code=" + getCode() +
        ", nonce=" + getNonce() + ", storage=" + getStorage() + '}';
  }
}
