...
public interface Geth extends Admin {
  ...
  Request<?, PersonalImportRawKey> personalImportRawKey(String keydata,
                                                        String password);

  Request<?, BooleanResponse> personalLockAccount(String accountId);

  Request<?, PersonalSign> personalSign(String message, String accountId,
                                        String password);

  Request<?, PersonalEcRecover> personalEcRecover(String message,
                                                  String signiture);
  ...
  /**
   * Creates an {@link Flowable} instance that emits a notification when a new
   * transaction is added to the pending state and is signed with a key that is
   * available in the node.
   *
   * @return a {@link Flowable} instance that emits a notification when a new
   *     transaction is added to the pending state
   */
  Flowable<PendingTransactionNotification>
  newPendingTransactionsNotifications();

  /**
   * Creates an {@link Flowable} instance that emits a notification when a node
