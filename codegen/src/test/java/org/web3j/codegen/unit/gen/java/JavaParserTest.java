...
public class JavaParserTest extends Setup {
  ...
  public void testGenerateJavaPoetStringTypesWhenReturnTypeIsNotContract() {
    ...
    Method newGreeting = listOfFilteredMethods.stream()
                             ...
                             .collect(Collectors.toList())
                             .get(0);
    JavaParser parser = new JavaParser(greeterContractClass, newGreeting,
                                       new JavaMappingHelper());
    assertEquals("$T $L = $L.newGreeting($S).send()",
                 parser.generatePoetStringTypes());
  }
  ...
  public void testGetMethodReturnType() {
    ...
    Method newGreeting = listOfFilteredMethods.stream()
                             ...
                             .collect(Collectors.toList())
                             .get(0);
    Parser parser = new JavaParser(greeterContractClass, newGreeting,
                                   new JavaMappingHelper());
