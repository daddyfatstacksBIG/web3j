...
public class KotlinParserTest extends Setup {
  ...
  public void testGenerateJavaPoetStringTypesWhenReturnTypeIsContract() {
    List<Method> listOfFilteredMethods =
        MethodFilter.extractValidMethods(greeterContractClass);
    Method deploy = listOfFilteredMethods.stream()
                        .filter(m -> m.getName().equals("deploy"))
                        .collect(Collectors.toList())
                        .get(0);
    KotlinParser parser = new KotlinParser(greeterContractClass, deploy,
                                           new KotlinMappingHelper());
    assertEquals(" %L = %T.deploy(%L, %L, %L, %S).send()",
                 parser.generatePoetStringTypes());
  }
  ...
  public void testGenerateJavaPoetStringTypesWhenReturnTypeIsNotContract() {
    ...
    List<Method> listOfFilteredMethods =
        MethodFilter.extractValidMethods(greeterContractClass);
    Method newGreeting = listOfFilteredMethods.stream()
                             .filter(m -> m.getName().equals("newGreeting"))
                             .collect(Collectors.toList())
                             .get(0);
    KotlinParser parser = new KotlinParser(greeterContractClass, newGreeting,
                                           new KotlinMappingHelper());
    assertEquals("val %L = %L.newGreeting(%S).send()",
                 parser.generatePoetStringTypes());
  }
  ...
  public void testGetMethodReturnType() {
    ...
    Method newGreeting = listOfFilteredMethods.stream()
                             ...
                             .collect(Collectors.toList())
                             .get(0);
    Parser parser = new KotlinParser(greeterContractClass, newGreeting,
                                     new KotlinMappingHelper());
