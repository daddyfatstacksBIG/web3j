...
public class SolidityFunctionWrapperGenerator extends FunctionWrapperGenerator {
  ...
  public static final String COMMAND_PREFIX =
      COMMAND_SOLIDITY + " " + COMMAND_GENERATE;
  ...
  protected SolidityFunctionWrapperGenerator(
      File binFile, File abiFile, File destinationDir, String contractName,
      String basePackageName, boolean useJavaNativeTypes,
      boolean useJavaPrimitiveTypes, int addressLength) {

    this(binFile, abiFile, destinationDir, contractName, basePackageName,
         useJavaNativeTypes, useJavaPrimitiveTypes, false, Contract.class,
         addressLength);
  }
  ...
  protected SolidityFunctionWrapperGenerator(
      File binFile, File abiFile, File destinationDir, String contractName,
      String basePackageName, boolean useJavaNativeTypes,
      boolean useJavaPrimitiveTypes, boolean generateSendTxForCalls,
      Class<? extends Contract> contractClass, int addressLength) {
  ...
  public final void generate() throws IOException, ClassNotFoundException {
    ...
    if (functionDefinitions.isEmpty()) {
      ...
      new SolidityFunctionWrapper(useJavaNativeTypes, useJavaPrimitiveTypes,
                                  generateSendTxForCalls, addressLength)
          .generateJavaFiles(
              contractClass, contractName, binary, functionDefinitions,
  ...
  @Command(name = COMMAND_PREFIX, mixinStandardHelpOptions = true,
           version = "4.0", sortOptions = false)
  private static class PicocliRunner implements Runnable {
    @Option(names = {"-a", "--abiFile"},
            description = "abi file with contract definition.", required = true)
    ...
    @Option(names = {"-o", "--outputDir"},
            description = "destination base directory.", required = true)
    ...
    @Option(names = {"-jt", JAVA_TYPES_ARG},
            description = "use native Java types.", required = false,
            showDefaultValue = ALWAYS)
    private boolean javaTypes = true;

    @Option(names = {"-st", SOLIDITY_TYPES_ARG},
            description = "use solidity types.", required = false)
    private boolean solidityTypes;

    @Option(names = {"-pt", PRIMITIVE_TYPES_ARG},
            description = "use Java primitive types.", required = false)
    private boolean primitiveTypes = false;

    @Override
    public void run() {
      try {
        // grouping is not implemented in picocli yet(planned for 3.1),
        // therefore simply check if solidityTypes were requested
        ...
        new SolidityFunctionWrapperGenerator(
            binFile, abiFile, destinationFileDir, contractName, packageName,
            useJavaTypes, primitiveTypes, addressLength)
            .generate();
      } catch (Exception e) {
        exitError(e);
      }
