...
public class ParserUtils {
  ...
  static Object[] generatePlaceholderValues(Method method, Class contract) {
    ...
    if (returnType.equals(contract)) {
      ...
      source1 = new Object[] {
          returnType, toCamelCase(returnTypeAsLiteral(returnType, true)),
  ...
  private static String getJavaPoetFormatSpecifier(Method method) {
    ...
    Arrays.asList(method.getParameterTypes())
        .forEach(
            type
            -> generated.add(
                mappingHelper.getJavaPoetFormat().getOrDefault(type, "$L")));
    return String.join(", ", generated);
  }

  static String generateAssertionJavaPoetStringTypes(Method method,
                                                     Class theContract) {
    Type returnType = getMethodReturnType(method);
    ...
    if (body[0].equals(TransactionReceipt.class)) {
      ...
      if (returnType.getTypeName().contains("Tuple")) {
        ...
        for (Type t : getTypeArray(returnType)) {
          symbolBuilder.append(mappingHelper.getJavaPoetFormat().get(t))
              .append(", ");
        }
  ...
  static Object[] generateAssertionPlaceholderValues(Method method,
                                                     Class contract) {
    Type returnType = getMethodReturnType(method);
  ...
  private static Object[] mergePlaceholderValues(Object[] source1,
                                                 Object[] source2) {
