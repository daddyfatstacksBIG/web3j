...
public class TupleGenerator extends Generator {
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (args.length == 1) {
      ...
      tupleGenerator.generate(System.getProperty("user.dir") +
                              "/tuples/src/main/java/");
    }
  }
  ...
  private TypeSpec createTuple(int size) {
    ...
    for (int i = 1; i <= size; i++) {
      ...
      MethodSpec getterSpec2 = MethodSpec.methodBuilder("component" + i)
                                   .addModifiers(Modifier.PUBLIC)
                                   .returns(typeVariableName)
                                   .addStatement("return $N", value)
                                   .build();
    ...
    return typeSpecBuilder.addJavadoc(buildWarning(TupleGenerator.class))
  ...
  private MethodSpec generateEqualsSpec(String className, int size) {
    ...
    equalsSpecBuilder.addStatement("$L $L = ($L) o", wildcardClassName, name,
                                   wildcardClassName);
    ...
    for (int i = 1; i < size; i++) {
      ...
      equalsSpecBuilder
          .beginControlFlow(
              "if ($L != null ? !$L.equals($L.$L) : $L.$L != null)", value,
    ...
    equalsSpecBuilder.addStatement(
        "return $L != null ? $L.equals($L.$L) : $L.$L == null", lastValue,
        lastValue, name, lastValue, name, lastValue);
  ...
  private MethodSpec generateHashCodeSpec(int size) {
    ...
    for (int i = 2; i <= size; i++) {
      ...
      hashCodeSpec.addStatement(
          "$L = 31 * $L + ($L != null ? $L.hashCode() : 0)", RESULT, RESULT,
          value, value);
    }
