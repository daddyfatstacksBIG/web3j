...
public class AbiTypesGenerator extends Generator {
  ...
  private static final String CODEGEN_WARNING =
      buildWarning(AbiTypesGenerator.class);
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length == 1) {
      ...
      abiTypesGenerator.generate(System.getProperty("user.dir") +
                                 "/abi/src/main/java/");
    }
  }
  ...
  private <T extends Type> void generateIntTypes(Class<T> superclass,
                                                 String path)
      throws IOException {
    ...
    for (int bitSize = 8; bitSize <= Type.MAX_BIT_LENGTH; bitSize += 8) {
      className =
          ClassName.get(packageName, superclass.getSimpleName() + bitSize);
      ...
      FieldSpec defaultFieldSpec =
          FieldSpec
              .builder(className, DEFAULT, Modifier.PUBLIC, Modifier.STATIC,
                       Modifier.FINAL)
      ...
      TypeSpec intType = TypeSpec.classBuilder(className.simpleName())
                             ...
                             .addMethods(Arrays.asList(constructorSpec,
                                                       overideConstructorSpec))
                             .build();
  ...
  private <T extends Type> void generateFixedTypes(Class<T> superclass,
                                                   String path)
      throws IOException {
    String packageName = createPackageName(superclass);
    ClassName className;
      ...
      for (int nBitSize = 8; nBitSize < Type.MAX_BIT_LENGTH; nBitSize += 8) {
        ...
        MethodSpec constructorSpec2 =
            MethodSpec.constructorBuilder()
                ...
                .addStatement("super($L, $L, $N, $N)", mBitSize, nBitSize, "m",
                              "n")
                .build();

        className = ClassName.get(packageName, superclass.getSimpleName() +
                                                   mBitSize + "x" + nBitSize);
        ...
        TypeSpec fixedType = TypeSpec.classBuilder(className.simpleName())
                                 .addJavadoc(CODEGEN_WARNING)
                                 .superclass(superclass)
                                 .addModifiers(Modifier.PUBLIC)
                                 .addField(defaultFieldSpec)
  ...
  private <T extends Type> void generateBytesTypes(String path)
      throws IOException {
    ...
    for (int byteSize = 1; byteSize <= 32; byteSize++) {
      ...
      className =
          ClassName.get(packageName, Bytes.class.getSimpleName() + byteSize);
      ...
      TypeSpec bytesType = TypeSpec.classBuilder(className.simpleName())
                               .addJavadoc(CODEGEN_WARNING)
  ...
  private <T extends Type> void generateStaticArrayTypes(String path)
      throws IOException {
    String packageName = createPackageName(StaticArray.class);
    ClassName className;

    for (int length = 1; length <= StaticArray.MAX_SIZE_OF_STATIC_ARRAY;
         length++) {
      ...
      className = ClassName.get(packageName,
                                StaticArray.class.getSimpleName() + length);
      ...
      TypeSpec bytesType =
          TypeSpec.classBuilder(className.simpleName())
              ...
              .addModifiers(Modifier.PUBLIC)
              .addMethods(Arrays.asList(oldConstructorSpec,
                                        oldArrayOverloadConstructorSpec))
              .addMethods(
