...
public abstract class ManagedTransactionTester {
  ...
  getVerifiedTransactionManager(Credentials credentials, int attempts,
                                int sleepDuration) {
  ...
  public TransactionManager
  getVerifiedTransactionManager(Credentials credentials) {
    RawTransactionManager transactionManager =
  ...
  void prepareNonceRequest() throws IOException {
    ...
    Request<?, EthGetTransactionCount> transactionCountRequest =
        mock(Request.class);
    when(transactionCountRequest.send()).thenReturn(ethGetTransactionCount);
    when(web3j.ethGetTransactionCount(SampleKeys.ADDRESS,
