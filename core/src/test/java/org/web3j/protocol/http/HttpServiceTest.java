...
public class HttpServiceTest {
  ...
  public void httpWebException() throws IOException {
    ...
    Request<String, EthBlockNumber> request =
        new Request<>("eth_blockNumber1", Collections.emptyList(),
                      mockedHttpService, EthBlockNumber.class);
    try {
      mockedHttpService.send(request, EthBlockNumber.class);
    } catch (ClientConnectionException e) {
      Assert.assertEquals(e.getMessage(), "Invalid response received: " +
  ...
  public void subscriptionNotSupported() {
    Request<Object, EthSubscribe> subscribeRequest = new Request<>(
        ...
        httpService, EthSubscribe.class);

    httpService.subscribe(subscribeRequest, "eth_unsubscribe",
