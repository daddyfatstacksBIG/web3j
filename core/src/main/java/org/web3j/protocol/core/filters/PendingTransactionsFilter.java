...
public class PendingTransactionsFilter extends Filter<List<String>> {
  ...
  protected void process(List<EthLog.LogResult> logResults) {
    ...
    for (EthLog.LogResult logResult : logResults) {
      if (!(logResult instanceof EthLog.Hash)) {
        throw new FilterException("Unexpected result type: " + logResult.get() +
                                  ", required Hash");
      }
  ...
  /**
   * Since the pending transaction filter does not support historic filters, the
   * filterId is ignored and an empty optional is returned
   *
