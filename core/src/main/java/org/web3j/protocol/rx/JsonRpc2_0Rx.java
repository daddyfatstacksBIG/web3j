...
public class JsonRpc2_0Rx {
  ...
  private <T> void run(org.web3j.protocol.core.filters.Filter<T> filter,
  ...
  replayBlocksFlowable(DefaultBlockParameter startBlock,
    ...
    return replayBlocksFlowable(startBlock, endBlock, fullTransactionObjects,
                                true);
  }
  ...
  replayBlocksFlowable(DefaultBlockParameter startBlock,
    ...
    // We use a scheduler to ensure this Flowable runs asynchronously for users
    // to be consistent with the other Flowables
    return replayBlocksFlowableSync(startBlock, endBlock,
                                    fullTransactionObjects, ascending)
        .subscribeOn(scheduler);
  }

  private Flowable<EthBlock>
  replayBlocksFlowableSync(DefaultBlockParameter startBlock,
                           DefaultBlockParameter endBlock,
                           boolean fullTransactionObjects) {
    return replayBlocksFlowableSync(startBlock, endBlock,
  ...
  private Flowable<EthBlock> replayBlocksFlowableSync(
      ...
      boolean containsFullTransactionObjects, boolean isAscending) {
    ...
    return Flowables.range(startBlockNumber, endBlockNumber, isAscending)
        ...
        .map(number
             -> web3j.ethGetBlockByNumber(number,
                                          containsFullTransactionObjects))
  ...
  replayPastBlocksFlowable(DefaultBlockParameter startBlock,
                           boolean fullTransactionObjects) {
    return replayPastBlocksFlowable(startBlock, fullTransactionObjects,
                                    Flowable.empty());
  }

  private Flowable<EthBlock>
  replayPastBlocksFlowableSync(DefaultBlockParameter startBlock,
                               boolean fullTransactionObjects,
                               Flowable<EthBlock> onCompleteFlowable) {
      ...
      return Flowable.concat(
          ...
          Flowable.defer(()
                             -> replayPastBlocksFlowableSync(
                                 ...
                                 fullTransactionObjects, onCompleteFlowable)));
    }
  }
  ...
  private BigInteger getBlockNumber(DefaultBlockParameter defaultBlockParameter)
    ...
    if (defaultBlockParameter instanceof DefaultBlockParameterNumber) {
      return ((DefaultBlockParameterNumber)defaultBlockParameter)
          .getBlockNumber();
    } else {
  ...
  private static List<Transaction> toTransactions(EthBlock ethBlock) {
    // If you ever see an exception thrown here, it's probably due to an
    // incomplete chain in Geth/Parity. You should resync to solve.
