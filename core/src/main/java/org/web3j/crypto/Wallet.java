 ...
 * href="https://github.com/ethereum/wiki/wiki/Web3-Secret-Storage-Definition">Web3
 * Secret Storage Definition</a> or the <a
 ...
 *
 * <p><strong>Note:</strong> the Bouncy Castle Scrypt implementation {@link
 * SCrypt}, fails to comply with the following Ethereum reference <a
 * href="https://github.com/ethereum/wiki/wiki/Web3-Secret-Storage-Definition#scrypt">Scrypt
 * test vector</a>:
 *
 * <pre>{@code
 ...
 * {
 *     throw new IllegalArgumentException("Cost parameter N_STANDARD must be > 1
 * and < 65536.");
 * }
 * }</pre>
 */
public class Wallet {
  ...
  public static WalletFile create(String password, ECKeyPair ecKeyPair, int n,
                                  int p) throws CipherException {

    byte[] salt = generateRandomBytes(32);

    byte[] derivedKey = generateDerivedScryptKey(password.getBytes(UTF_8), salt,
                                                 n, R, p, DKLEN);
    ...
    byte[] cipherText = performCipherOperation(Cipher.ENCRYPT_MODE, iv,
                                               encryptKey, privateKeyBytes);
  ...
  private static WalletFile createWalletFile(ECKeyPair ecKeyPair,
                                             byte[] cipherText, byte[] iv,
                                             byte[] salt, byte[] mac, int n,
                                             int p) {
  ...
  private static byte[] generateDerivedScryptKey(byte[] password, byte[] salt,
                                                 int n, int r, int p, int dkLen)
      throws CipherException {
    return SCrypt.generate(password, salt, n, r, p, dkLen);
  }

  private static byte[] generateAes128CtrDerivedKey(byte[] password,
                                                    byte[] salt, int c,
                                                    String prf)
      throws CipherException {
    ...
    // array, see http://stackoverflow.com/a/27928435/3211687
  ...
  public static ECKeyPair decrypt(String password, WalletFile walletFile)
    ...
    if (kdfParams instanceof WalletFile.ScryptKdfParams) {
      ...
      derivedKey = generateDerivedScryptKey(password.getBytes(UTF_8), salt, n,
                                            r, p, dklen);
      ...
      derivedKey =
          generateAes128CtrDerivedKey(password.getBytes(UTF_8), salt, c, prf);
    } else {
      throw new CipherException("Unable to deserialize params: " +
                                crypto.getKdf());
    }
  ...
  static void validate(WalletFile walletFile) throws CipherException {
    ...
    if (!crypto.getKdf().equals(AES_128_CTR) &&
        !crypto.getKdf().equals(SCRYPT)) {
