...
/**
 * Transaction receipt processor that uses a single thread to query for
 * transaction receipts.
 *
 * <p><em>Note:</em>When initially invoked, this processor returns a transaction
 * receipt containing only the transaction hash of the submitted transaction.
 * This is encapsulated in an {@link EmptyTransactionReceipt}.
 */
public class QueuingTransactionReceiptProcessor
  ...
  public QueuingTransactionReceiptProcessor(Web3j web3j, Callback callback,
    ...
    scheduledExecutorService.scheduleAtFixedRate(
        this::sendTransactionReceiptRequests, pollingFrequency,
        pollingFrequency, TimeUnit.MILLISECONDS);
  }
          ...
          if (requestWrapper.getCount() == pollingAttemptsPerTxHash) {
            throw new TransactionException(
                "No transaction receipt for txHash: " + transactionHash +
                    "received after " + pollingAttemptsPerTxHash + " attempts",
                transactionHash);
          } else {
  ...
  /**
   * Java doesn't provide a concurrent linked hash set, so we use a simple
   * wrapper to store details of the number of requests we've made against this
   * specific transaction hash. This is so we can preserve submission order as
   * we interate over the outstanding transactions.
   *
   * <p>Note - the equals/hashcode methods only operate on the transactionHash
  ...
  private static class RequestWrapper {
    ...
    String getTransactionHash() { return transactionHash; }
